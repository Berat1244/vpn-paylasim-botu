...
# --- WEB SERVER (Render Uyanık Kalsın Diye) ---
app = Flask('')
@app.route('/')
...
# Android Dosya Seçimi (+ Geri Tuşu)
def android_files():
    markup = types.InlineKeyboardMarkup()
...
def callback_query(call):
    # Anamenüye Dönüş Metni
    ana_metin = "👋 Hoş Geldin!\n\nBu bot ile VPN dosyalarını kolayca alabilirsin ✅\n\nDosya almak için aşağıdaki butonları kullan 🎉"

    if call.data == "main_don":
        bot.edit_message_text(ana_metin, call.message.chat.id, call.message.message_id, reply_markup=main_menu())
    ...
    # DOSYA KONTROL VE GÖNDERME
        ...
        # Dosya GitHub'da yüklü mü kontrol et
        if os.path.exists(file_name):
            bot.answer_callback_query(call.id, "Dosya gönderiliyor...")
            with open(file_name, 'rb') as doc:
                bot.send_document(call.message.chat.id, doc, caption=f"✅ **{file_name}** Hazır!")
        else:
            # Dosya yoksa senin istediğin uyarıyı ver
